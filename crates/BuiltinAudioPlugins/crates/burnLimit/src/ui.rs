//! Embedded editor UI for BurnLimit.
//!
...
//! through the `mikoplugin://burnlimit/...` scheme; see [`builtin_ui_embed`]
...
pub const UI_ORIGIN: &str = "burnlimit";

/// Marker type carrying the embedded editor assets.
pub struct BurnLimitUi;
...
impl EmbeddedPluginUi for BurnLimitUi {
...
mod tests {
    ...
    fn lookups_are_total_and_never_panic() {
        for path in [
            ...
            let _ = BurnLimitUi::get_ui_asset(path);
            let _ = BurnLimitUi::resolve_ui_asset(path);
        }
    }

    #[test]
    fn traversal_is_rejected_even_when_a_table_is_present() {
        assert!(BurnLimitUi::get_ui_asset("/../../etc/passwd").is_none());
        assert!(BurnLimitUi::resolve_ui_asset("/../../etc/passwd").is_none());
    }

    #[test]
    fn a_built_dist_serves_index_html_and_round_trips_every_entry() {
        if !BurnLimitUi::is_embedded() {
            return;
        }
        let index = BurnLimitUi::get_ui_asset("/index.html")
            .expect("an embedded table always contains /index.html");
        assert!(!index.is_empty());
        assert!(index.mime_type.starts_with("text/html"));

        assert_eq!(
            BurnLimitUi::resolve_ui_asset("/").map(|a| a.path),
            Some("/index.html")
        );
        assert_eq!(
            BurnLimitUi::resolve_ui_asset("").map(|a| a.path),
