...
//! `cargo test -p burnlimit` working for anyone who has not run
...
fn main() {
    ...
    match builtin_ui_embed::generate::generate(&options) {
        ...
        Ok(_) => {
            println!(
                "cargo:warning=burnlimit: editorui/dist not built; the plugin editor \
                 will have no UI assets (run `bun run build` in editorui/)"
            );
        }
        Err(error) => panic!("burnlimit: failed to embed editor UI: {error}"),
    }
}
