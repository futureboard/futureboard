/**
 * Display mapping for BurnLimit's Pro-L–style stage.
 *
...
export function levelNorm(db: number): number {
  return clamp(
    (STAGE_CEIL_DB - db) / (STAGE_CEIL_DB - STAGE_FLOOR_DB),
    0,
    1,
  )
}
...
/**
 * Gain reduction as a fraction of the stage height growing downward from
 * the top (0 dB line), matching Pro-L's red overlay.
 */
...
/**
 * One-pole ballistics for meter fill (not VU hardware). Fast enough to feel
 * alive at ~30 Hz host telemetry without inventing peaks.
 */
...
/**
 * Whether a GR spike is worth tagging — mirrors Pro-L's peak labels without
 * inventing reduction that wasn't measured.
 */
