...
/** Variant order of `enum Style`, which is the `to_wire` contract. */
export function rustStyleOrder(): string[] {
  const match = /pub enum Style \{([\s\S]*?)\n\}/.exec(LIB_RS)
  if (!match?.[1]) throw new Error('enum Style not found')
