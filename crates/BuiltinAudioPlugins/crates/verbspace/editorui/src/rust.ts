...
/**
 * The `match self { Self::Room => 0.62, ... }` arms of a `ReverbMode` method,
 * keyed by the lowercased variant name.
 */
export function rustModeArms(method: string): Record<string, number> {
  const fn = new RegExp(
    `fn ${method}\\(self\\)\\s*->\\s*f32\\s*\\{([\\s\\S]*?)\\n    \\}`,
  ).exec(LIB_RS)
  if (!fn?.[1]) throw new Error(`ReverbMode::${method} not found`)
  const arms: Record<string, number> = {}
  for (const [, variant, value] of fn[1].matchAll(
    /Self::(\w+)\s*=>\s*([0-9_.]+)/g,
  )) {
    arms[variant!.toLowerCase()] = num(value!)
  }
  return arms
}

/** Variant order of `enum ReverbMode`, which is the `to_wire` contract. */
export function rustModeOrder(): string[] {
  const match = /pub enum ReverbMode \{([\s\S]*?)\n\}/.exec(LIB_RS)
  if (!match?.[1]) throw new Error('enum ReverbMode not found')
