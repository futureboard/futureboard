...
//! `cargo test -p equzx` working for anyone who has not run `bun run build`
//! in `editor/`.
...
fn main() {
    ...
    match builtin_ui_embed::generate::generate(&options) {
        ...
        Ok(report) if report.dist_present => {
            println!(
                "cargo:rustc-env=EQUZX_UI_ASSET_COUNT={}",
                report.asset_count
            );
        }
        Ok(_) => {
            ...
            println!(
                "cargo:warning=equzx: editor/dist not built; the plugin editor \
                 will have no UI assets (run `bun run build` in editor/)"
            );
        }
        Err(error) => panic!("equzx: failed to embed editor UI: {error}"),
    }
}
