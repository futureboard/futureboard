//! Embedded editor UI for Transient.
//!
...
//! through the `mikoplugin://transient/...` scheme; see [`builtin_ui_embed`]
...
impl EmbeddedPluginUi for TransientUi {
...
mod tests {
    ...
    fn lookups_are_total_and_never_panic() {
        for path in [
            ...
            let _ = TransientUi::get_ui_asset(path);
            let _ = TransientUi::resolve_ui_asset(path);
        }
    }

    #[test]
    fn traversal_is_rejected_even_when_a_table_is_present() {
        assert!(TransientUi::get_ui_asset("/../../etc/passwd").is_none());
        assert!(TransientUi::resolve_ui_asset("/../../etc/passwd").is_none());
    }

    #[test]
    fn a_built_dist_serves_index_html_and_round_trips_every_entry() {
        if !TransientUi::is_embedded() {
            return;
        }
        let index = TransientUi::get_ui_asset("/index.html")
            .expect("an embedded table always contains /index.html");
        assert!(!index.is_empty());
        assert!(index.mime_type.starts_with("text/html"));

        assert_eq!(
            TransientUi::resolve_ui_asset("/").map(|a| a.path),
            Some("/index.html")
        );
        assert_eq!(
            TransientUi::resolve_ui_asset("").map(|a| a.path),
