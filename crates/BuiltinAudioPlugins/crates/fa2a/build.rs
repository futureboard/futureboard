...
//! `cargo test -p fa2a` working for anyone who has not run
...
fn main() {
    ...
    match builtin_ui_embed::generate::generate(&options) {
        ...
        Ok(report) if report.dist_present => {
            println!("cargo:rustc-env=FA2A_UI_ASSET_COUNT={}", report.asset_count);
        }
        Ok(_) => {
            // Not fatal — the DSP core is independently useful and tested.
            println!(
                "cargo:warning=fa2a: editorui/dist not built; the plugin editor \
                 will have no UI assets (run `bun run build` in editorui/)"
            );
        }
        Err(error) => panic!("fa2a: failed to embed editor UI: {error}"),
    }
}
