...
//! through the `mikoplugin://fa2a/...` scheme; see [`builtin_ui_embed`]
...
impl EmbeddedPluginUi for Fa2aUi {
...
mod tests {
    ...
    fn lookups_are_total_and_never_panic() {
        for path in [
            ...
            let _ = Fa2aUi::get_ui_asset(path);
            let _ = Fa2aUi::resolve_ui_asset(path);
        }
    }

    #[test]
    fn traversal_is_rejected_even_when_a_table_is_present() {
        assert!(Fa2aUi::get_ui_asset("/../../etc/passwd").is_none());
        assert!(Fa2aUi::resolve_ui_asset("/../../etc/passwd").is_none());
    }

    #[test]
    fn a_built_dist_serves_index_html_and_round_trips_every_entry() {
        if !Fa2aUi::is_embedded() {
            // dist/ not built in this checkout — nothing to assert.
            return;
        }
        let index = Fa2aUi::get_ui_asset("/index.html")
        ...
        assert_eq!(
            Fa2aUi::resolve_ui_asset("/").map(|a| a.path),
            Some("/index.html")
        );
        assert_eq!(
            Fa2aUi::resolve_ui_asset("").map(|a| a.path),
