/**
 * Editor-side mirror of FA-2A's parameter contract.
 *
...
export const PARAMS: Record<ParamId, ParamSpec> = {
  peakReduction: spec('peakReduction', 'Peak Reduction', 0, 100, 35, '', 'lin', 0, 1),
  gainDb: spec('gainDb', 'Gain', -12, 24, 0, 'dB', 'lin', 1, 0.5, 0),
  emphasis: spec('emphasis', 'Emphasis', 0, 100, 45, '', 'lin', 0, 1),
  mix: spec('mix', 'Mix', 0, 100, 100, '%', 'lin', 0, 1),
  color: spec('color', 'Color', 0, 100, 12, '%', 'lin', 0, 1),
  sidechainLowCutHz: spec(
    'sidechainLowCutHz',
    'Sidechain',
    20,
    500,
  ...
  outputTrimDb: spec('outputTrimDb', 'Trim', -12, 12, 0, 'dB', 'lin', 1, 0.5, 0),
}

export const MODES = ['compress', 'limit'] as const
export type Mode = (typeof MODES)[number]

/** Panel legend, in the hardware's own wording. */
...
export function modeToWire(mode: Mode): number {
  ...
  return index < 0 ? MODES.indexOf('compress') : index
}

export function modeFromWire(value: number): Mode {
  return MODES[Math.round(value)] ?? 'compress'
}
