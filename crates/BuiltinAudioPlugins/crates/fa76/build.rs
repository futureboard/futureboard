...
//! `cargo test -p fa76` working for anyone who has not run
...
fn main() {
    ...
    match builtin_ui_embed::generate::generate(&options) {
        ...
        Ok(report) if report.dist_present => {
            println!("cargo:rustc-env=FA76_UI_ASSET_COUNT={}", report.asset_count);
        }
        Ok(_) => {
            // Not fatal — the DSP core is independently useful and tested.
            println!(
                "cargo:warning=fa76: editorui/dist not built; the plugin editor \
                 will have no UI assets (run `bun run build` in editorui/)"
            );
        }
        Err(error) => panic!("fa76: failed to embed editor UI: {error}"),
    }
}
