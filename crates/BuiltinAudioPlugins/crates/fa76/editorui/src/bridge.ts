/**
 * Native bridge for the FA-76 editor.
 *
...
import { RATIOS, type Ratio } from './params'

export const BRIDGE_PROTOCOL_VERSION = 1
export const PLUGIN_ID = 'fa76'

/** The state blob `fa76::ipc::Fa76State` serializes. */
export type Fa76Params = {
  power: boolean
  ratio: Ratio
  inputDb: number
  outputDb: number
 ...
 *
 * `gainReductionDb` is positive — how much the FET cell is taking off right
...
function parseParams(state: unknown): Fa76Params | null {
  ...
  if (!RATIOS.includes(params.ratio as Ratio)) return null
  for (const key of NUMERIC_KEYS) {
    const value = params[key]
    if (typeof value !== 'number' || !Number.isFinite(value)) return null
  }
  return params as unknown as Fa76Params
}

export function connectBridge(
  onParams: (params: Fa76Params) => void,
