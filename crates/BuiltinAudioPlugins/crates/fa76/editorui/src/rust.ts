...
/** Variant order of `enum RatioButton`, which is the `to_wire` contract. */
export function rustRatioOrder(): string[] {
  const match = /pub enum RatioButton \{([\s\S]*?)\n\}/.exec(LIB_RS)
  if (!match?.[1]) throw new Error('enum RatioButton not found')
