...
//! through the `mikoplugin://fa76/...` scheme; see [`builtin_ui_embed`]
...
impl EmbeddedPluginUi for Fa76Ui {
...
mod tests {
    ...
    fn lookups_are_total_and_never_panic() {
        for path in [
            ...
            let _ = Fa76Ui::get_ui_asset(path);
            let _ = Fa76Ui::resolve_ui_asset(path);
        }
    }

    #[test]
    fn traversal_is_rejected_even_when_a_table_is_present() {
        assert!(Fa76Ui::get_ui_asset("/../../etc/passwd").is_none());
        assert!(Fa76Ui::resolve_ui_asset("/../../etc/passwd").is_none());
    }

    #[test]
    fn a_built_dist_serves_index_html_and_round_trips_every_entry() {
        if !Fa76Ui::is_embedded() {
            // dist/ not built in this checkout — nothing to assert.
            return;
        }
        let index = Fa76Ui::get_ui_asset("/index.html")
        ...
        assert_eq!(
            Fa76Ui::resolve_ui_asset("/").map(|a| a.path),
            Some("/index.html")
        );
        assert_eq!(
            Fa76Ui::resolve_ui_asset("").map(|a| a.path),
